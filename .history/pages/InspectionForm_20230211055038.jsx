...
const InspectionForm = () => {
  ...
  const uploadToClient = event => {
    if (event.target.files && event.target.files[0]) {
      ...
      const formattedDate = `${currentDate.getDate()}_${currentDate.toLocaleString('default', {
        month: 'short'
      })}_${currentDate.getFullYear()}`;
      const newFileName = `${i.name.split('.')[0]}_${formattedDate}.${i.name.split('.')[1]}`;

      const uniqueFile = new File([i], newFileName, { type: i.type });
      setImage(uniqueFile);
      setCreateObjectURL(URL.createObjectURL(uniqueFile));
    }
  };
  ...
  return (
    <>
          ...
          <table className="table table-bordered ">
            ...
            <tbody>
              ...
              <tr>
                {/* <td colSpan={2}>
                  <div>
                    <FileInput onImageChange={setFile} />
                  </div>
                </td>
                <td colSpan={1}>
                  <div className="text-center">{file && <FileUpload file={file} />}</div>
                </td> */}
                <td>
                  <div>
                    <div>
                      <div>{image ? <h1>{image.name}</h1> : <p>no file selected</p>}</div>
                      <div>{image ? <img src={createObjectURL} /> : null}</div>
      ...
      {!user ||
        ...
        (!user[ROLES].includes('TCO') && (
          <>
            <h3 className="text-center text-warning m-5">You don't have permissions to access this page</h3>
            <p className="text-center text-warning m-5">Request for permissions</p>
          </>
        ))}
    </>
  );
