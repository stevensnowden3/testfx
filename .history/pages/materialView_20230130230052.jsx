...
const role = user['https://netlify-integration.com/roles'].includes('TCO') ? 1 :
user['https://netlify-integration.com/roles'].includes('QAM') ? 2 :
user['https://netlify-integration.com/roles'].includes('LWM') ? 3 : 0;

if (role) {
try {
await axios.patch(/api/checklist/${router.query.id}, { status: role });
setStatus(role);
} catch (error) {
console.log(error);
}
          ...
          {item ? (
            <div className=" container m-5">
              ...
              <div className="text-center mt-5 ">
                ...
                {status === 1 && (
                  <p className="text-success">Approved on {moment(item.updatedAt).format('DD/MM/YYYY')}</p>
                )}
              </div>
