  ...
  const renderAction = (rowData, column) => {
    return (
      <Link href={{ pathname: '/materialView', query: { id: rowData.id } }}>
        <Button label="View" style={{ background: '#04392a', color: '#fff', fontSize: '12px' }} />
      </Link>
    );
  };
      ...
      {(user && user[ROLES].includes('TCO')) || user[ROLES].includes('QAM') || user[ROLES].includes('LWM') ? (
        <div>
          <div className="container-fluid">
            <h3 className="text-center p-3 mb-2" style={{ background: '#b5bd36', color: '#fff' }}>
          ...
          <DataTable value={filterData()} paginator rows={7} responsive>
            ...
            <Column
              ...
              headerStyle={{ backgroundColor: '#04392a', color: '#fff' }}
            />
            <Column header="Actions" headerStyle={{ backgroundColor: '#04392a', color: '#fff' }} body={renderAction} />
          </DataTable>
          <div className="m-3">
