...
const View = () => {
  ...
  // All Variables
  const [material_name, setMaterialName] = useState(item ? item.material_name : '');
  const [supplier_name, setSupplierName] = useState(item ? item.supplier_name : '');
  const [method_of_inspection, setMethodOfInspection] = useState(item ? item.method_of_inspection : '');
  const [material_received, setcontractNo] = useState(item ? item.material_received : '');
  const [total_inspected, setTotalInspected] = useState(item ? item.total_inspected : '');
  const [total_pass, setTotalPass] = useState(item ? item.total_pass : '');
  const [total_failure, setTotalFailure] = useState(item ? item.total_failure : '');
  const [delivery_note, setDeliveryNote] = useState(item ? item.delivery_note : '');
  const [sample_inspected, invoiceNo] = useState(item ? item.sample_inspected : '');
  const [delivery_date, setDeliveryDate] = useState(item ? new Date(item.delivery_date) : new Date());
  const [inspection_date, setSelectedInspectionDate] = useState(item ? new Date(item.inspection_date) : new Date());
  // dates
                        ...
                        <tr>
                          <td>
                            ...
                            <span className="me-2 text-warning">
                              {method_of_inspection ? method_of_inspection.name : 'none'}
                            </span>
                          </td>
                          <td>
                            Sample:{' '}
                            <span className="me-2 text-warning">{material_name ? material_name.name : 'none'}</span>
                          </td>
                          <td>
                            InsDate
                          ...
                          <td>
                            ...
                            <span className="text-warning m-2 ">
                              {delivery_date ? moment(delivery_date).fromNow() : 'none'}
                            </span>
                          </td>
                          <td>
