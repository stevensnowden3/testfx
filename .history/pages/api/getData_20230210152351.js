...
export default withApiAuthRequired(async (req, res) => {
  ...
  try {
    ...
    if (req.query.approved === 'TCO') {
      ...
      const [rows] = await connection.execute(
        'SELECT id,material_name, supplier_name,method_of_inspection,inspection_technique,location,material_received, total_inspected, total_pass, total_failure, delivery_note,sample_inspected,status, inspection_date,created_by FROM checklisttest WHERE status=4'
      );
