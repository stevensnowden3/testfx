...
const View = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setMessage({ type: 'success', text: response.data.data[0].message });
      console.log('sucess!');
    } catch (error) {
      setMessage({ type: 'error', text: error.response.data.data[0].message });
      console.log('fail!');
    }
  };
          ...
          <>
            <form onSubmit={handleSubmit}>
              <div>
                ...
                <input type="text" name="material_name" value={formData.material_name} onChange={handleChange} />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" name="total_inspected" value={formData.total_inspected} onChange={handleChange} />
              </div>
              <div>
                <label htmlFor="total_pass">Total Pass:</label>
                <input type="text" name="total_pass" value={formData.total_pass} onChange={handleChange} />
              </div>
              <div>
                <label htmlFor="total_failure">Total Failure:</label>
                <input type="text" name="total_failure" value={formData.total_failure} onChange={handleChange} />
              </div>
              <div>
                <label htmlFor="delivery_note">Delivery Note:</label>
                <input type="text" name="delivery_note" value={formData.delivery_note} onChange={handleChange} />
              </div>
              <div>
                <label htmlFor="sample_inspected">Sample Inspected:</label>
                <input type="text" name="sample_inspected" value={formData.sample_inspected} onChange={handleChange} />
              </div>
              <div>
