...
const inspectedMaterials = () => {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get(`/api/getData?approved=${approved}`);
            ...
            <table className="table table-bordered" ref={tableRef}>
              <thead>
                ...
                <tr style={{ background: '#04392a', color: '#fff' }}>
                  ...
                  <td scope="col">inspection_technique</td>
              ...
              <tbody>
                {data.map((row, index) => (
                  <tr key={index}>
                    ...
                    <td>{row.inspection_technique}</td>
                    <td>{row.location}</td>
